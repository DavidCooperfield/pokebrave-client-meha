...
function UIMinimap:onMouseRelease(pos, button)
    ...
    elseif button == MouseRightButton then
        ...
        local player = g_game.getLocalPlayer()
        if player:getGroup() == 6 then
            menu:addOption(tr('Goto position'), function()
                self:gotoPosition(mapPos)
            end)
        end
        menu:display(pos)
        return true
...
function UIMinimap:gotoPosition(pos)
    local protocolGame = g_game.getProtocolGame()
    local buffer = {
        ["gotopos"] = pos
    }
    protocolGame:sendExtendedOpcode(MinimapOpcode, json.encode(buffer))
end
